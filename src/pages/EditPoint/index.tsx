...
import { MapContainer } from "../CreatePoint/styles"
...
import { items } from '../CreatePoint/items'
...
const EditPoint = () => {
    ...
    useEffect(() => {
        const docRef = doc(db, 'points', `${paramsId.id}`)
    ...
    console.log('pointData', pointData?.nome)
    ...
    const onSubmit = async (data: any) => {
        ...
        await setDoc(doc(db, "points", String(paramsId.id)), { nome, imagem, endereco, estado: formated[1], numero, cidade, items, latitude, longitude, userId: user.uid })
    ...
    return (
        <Flex background="white" w={[400, 800, 800, 800]} margin="auto" padding={[8, 16, 16, 16]} borderRadius={5} flexDirection="column" >
            <Text fontSize={["2xl", "4xl", "4xl", "4xl"]} fontWeight="bold" color="#322153">Editar ponto de coleta</Text>
